...
const UpdateWorkAddressModal: FC<TUpdateWorkAddressModalProps> = ({
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast.success('User Work Address updated');
      setTimeout(() => {
        setModalType(null);
      }, 1000);
    },
    onError: (error: AxiosError) => {
      console.error('User Work Address update failed:', error);
      toast.dismiss();
      toast.error(
        (error?.response?.data as { message?: string })?.message ||
          'Failed update User Work Address. Please try again.'
      );
    },
  });
  ...
  const handleSaveChanges = () => {
    ...
    const payload: IProfileUpdatePayload = {
      location: { work: userWorkAddress, home: home as string },
    };
    mutate(payload);
  };
  useEffect(() => {
    setIsChangeMade((work || '') !== userWorkAddress);
  }, [userWorkAddress, work]);
  useEffect(() => {
          ...
          <div className="space-y-3">
            ...
            <textarea
              ...
              onChange={(e: ChangeEvent<HTMLTextAreaElement>) => {
                ...
                setUserWorkAddress(value);
              }}
              name="home_address"
            />
            <p className="text-xs text-slate-500">
              Your work address is used for professional correspondence and
              deliveries.
            </p>
          </div>
        </div>
